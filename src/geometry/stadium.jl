...
function Stadium(half_width)
    ...
    l_seg = SymLineSegment(SVector(0.0,1.0),SVector(0.0,0.0))
    b_seg = SymLineSegment(SVector(0.0,0.0),SVector(half_width,0.0))
    ...
    return Stadium{typeof(half_width)}([circle_dom,rectangle_dom])
end
